...
import type { ChoiceOption, ColumnSchema, FieldSchema, FkValue, RowValue } from '@/lib/admin-kit/types'
...
export function CellValue({ column, value }: { column: ColumnSchema; value: RowValue | undefined }) {
  ...
  switch (column.type) {
    ...
    case 'datetime':
      return <span className="text-muted-foreground">{new Date(String(value)).toLocaleString()}</span>
    case 'date':
      return <span className="text-muted-foreground">{new Date(String(value)).toLocaleDateString()}</span>
    case 'decimal':
    case 'integer':
    ...
    default:
      return <span className="max-w-[28ch] truncate align-middle">{String(value)}</span>
  }
}
...
export function FieldInput({ field, value, onChange, fkOptions, error }: FieldInputProps) {
  ...
  const control = (() => {
    switch (field.type) {
      case 'boolean':
        return <KitToggle checked={Boolean(value)} onChange={onChange} disabled={disabled} label={field.label} />
      case 'text':
        return (
          <KitTextarea
            value={String(value ?? '')}
  ...
  return (
    <div className="space-y-1.5">
      <label className="flex items-center gap-1 text-sm font-medium text-foreground">
        {field.label}
        {field.required && !disabled && <span className="text-destructive">*</span>}
        {disabled && <span className="text-xs font-normal text-muted-foreground">(read-only)</span>}
      </label>
      {control}
      {error ? (
