...
import type { ActionSchema, ChoiceOption, ListPage, ModelMeta, Row } from '@/lib/admin-kit/types'

import { ModelForm } from './model-form'
import { ModelList } from './model-list'
import { KitBanner, KitButton, KitInput, KitSelect, KitSkeletonRows } from './primitives'
...
export function AdminModelPage({ apiBase, modelKey }: { apiBase: string; modelKey: string }) {
  ...
  const [fkFilterOptions, setFkFilterOptions] = useState<Record<string, ChoiceOption[]>>({})
      ...
      .then((m) => {
        ...
        for (const filter of m.filters) {
          ...
          client
            ...
            .then(({ results }) => {
              if (!cancelled) setFkFilterOptions((prev) => ({ ...prev, [filter.name]: results }))
            })
            .catch(() => undefined)
        }
      })
      .catch((err) => {
        if (!cancelled) setLoadError(err instanceof AdminKitError ? err.detail : 'Failed to load model.')
      })
    return () => {
      cancelled = true
    }
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((err) => {
        if (!cancelled) setLoadError(err instanceof AdminKitError ? err.detail : 'Failed to load rows.')
      })
    return () => {
      cancelled = true
    }
  ...
  const showBanner = useCallback((kind: 'success' | 'error', message: string) => {
    ...
    if (kind === 'success') bannerTimer.current = setTimeout(() => setBanner(null), 4000)
  }, [])
  ...
  const applyActionResult = (result: { detail?: string; redirect?: string }) => {
    if (result.redirect) {
  ...
  const runBulkAction = async (name: string) => {
    ...
    setBusyAction(name)
    try {
      applyActionResult(await client.runAction(modelKey, name, Array.from(selected)))
    } catch (err) {
      showBanner('error', err instanceof AdminKitError ? err.detail : 'Action failed.')
    } finally {
      setBusyAction('')
    }
  }
  ...
  if (loadError) {
    return (
      <div className="p-4 md:p-6">
        <KitBanner kind="error" message={loadError} onDismiss={() => setLoadError('')} />
      </div>
    )
  }
  ...
  const totalPages = page ? Math.max(1, Math.ceil(page.count / meta.page_size)) : 1

  return (
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">{meta.label_plural}</h1>
          {meta.description && <p className="mt-1 max-w-xl text-sm text-muted-foreground">{meta.description}</p>}
        </div>
        {meta.can_create && (
          <KitButton variant="primary" onClick={() => setForm({ mode: 'create' })}>
            <Plus className="h-4 w-4" /> New {meta.label}
          </KitButton>
        )}
      </div>

      {banner && <KitBanner kind={banner.kind} message={banner.message} onDismiss={() => setBanner(null)} />}
          ...
          filter.type === 'string' ? (
            ...
            >
              ...
              {filter.type === 'boolean' ? (
                ...
                (filter.choices ?? fkFilterOptions[filter.name] ?? []).map((option) => (
      ...
      <div className="rounded-lg border bg-card">
        {page === null ? (
          ...
          <ModelList
            ...
            onToggleAll={() =>
              setSelected((prev) => {
                const pks = page.results.map((row) => String(row[meta.pk_field]))
                return pks.every((pk) => prev.has(pk)) ? new Set() : new Set(pks)
              })
            }
      ...
      {page && page.count > 0 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>
            {page.count} {(page.count === 1 ? meta.label : meta.label_plural).toLowerCase()}
          </span>
          {totalPages > 1 && (
            <div className="flex items-center gap-2">
              <KitButton className="h-8 px-2" disabled={pageNum <= 1} onClick={() => setPageNum(pageNum - 1)}>
