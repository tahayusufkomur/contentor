...
import type { ChoiceOption, FieldSchema, ModelMeta, Row, RowValue } from '@/lib/admin-kit/types'
...
function initialValue(field: FieldSchema, row: Row | null): unknown {
  ...
  if (field.default !== undefined) return field.type === 'json' ? JSON.stringify(field.default) : field.default
...
export function ModelForm({
  ...
  const editable = useMemo(() => meta.form_fields.filter((f) => !f.read_only), [meta])
  const visibleFields = isCreate ? editable : meta.form_fields

  const [values, setValues] = useState<Record<string, unknown>>(() =>
    Object.fromEntries(visibleFields.map((f) => [f.name, initialValue(f, row)])),
  )
  ...
  useEffect(() => {
    ...
    for (const field of editable) {
      ...
      client
        ...
        .then(({ results }) => {
          if (!cancelled) setFkOptions((prev) => ({ ...prev, [field.name]: results }))
        })
        .catch(() => undefined)
    }
    return () => {
  ...
  const remove = async () => {
    if (!row) return
    if (!window.confirm(`Delete this ${meta.label.toLowerCase()}? This cannot be undone.`)) return
    setBusy(true)
    setFormError('')
    try {
      ...
      setFormError(err instanceof AdminKitError ? err.detail : 'Delete failed.')
      setBusy(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative flex h-full w-full max-w-md flex-col border-l bg-background shadow-xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          ...
          <KitButton variant="ghost" aria-label="Close panel" onClick={onClose} className="h-8 px-2">
            <X className="h-4 w-4" />
          </KitButton>
        </div>
