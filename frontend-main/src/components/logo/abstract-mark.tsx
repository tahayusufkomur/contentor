// MIRRORED FROM frontend-customer/src/components/logo/abstract-mark.tsx — keep in sync (phase-3 wizard)
      ...
      {shapes.map((s, i) => {
        if (s.kind === "circle") {
          return s.stroke ? (
            <circle
              ...
              fill="none"
              stroke={color}
              strokeWidth={(s.strokeWidth ?? 0.04) * size}
              opacity={s.opacity}
            />
          ) : (
            <circle
        ...
        if (s.kind === "line") {
          return (
            <line
              ...
              y2={s.y2 * size}
              stroke={color}
              strokeWidth={s.strokeWidth * size}
              opacity={s.opacity}
              strokeLinecap="round"
            />
          );
        ...
        return s.stroke ? (
          <path
            ...
            fill="none"
            stroke={color}
            strokeWidth={s.strokeWidth ?? 0.04}
            opacity={s.opacity}
            strokeLinecap="round"
          />
        ) : (
