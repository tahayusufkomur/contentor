...
export function MiniNavbar({
  ...
  if (layout === "pill") {
    return (
      <div
        className="flex h-10 w-full items-center justify-center text-[10px]"
        style={{ color: s.ink }}
      >
        <div
...
export function MiniHero({
  ...
  if (style === "split") {
    return (
      <div
        className="flex h-28 w-full gap-2 rounded-lg border p-2"
        style={{ borderColor: `${s.primary}33`, background: "white" }}
      >
        <div className="flex flex-1 flex-col justify-center gap-1.5">
          <span
            className="line-clamp-2 text-[10px] font-bold leading-tight"
            style={{ color: s.ink, fontFamily: fontStack(font) }}
          >
            {title}
          </span>
          <span
            className="h-3 w-12 rounded-full text-center text-[7px] font-semibold leading-3 text-white"
            style={{ background: s.primary }}
          >
            CTA
          </span>
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${s.soft}, ${s.primary}66)`,
          }}
        />
      </div>
    );
  ...
  if (style === "minimal") {
    return (
      <div
        className="flex h-28 w-full flex-col items-center justify-center gap-1.5 rounded-lg border"
        style={{ borderColor: `${s.primary}33`, background: "white" }}
      >
        <span
          className="px-3 text-center text-[10px] font-bold leading-tight"
          style={{ color: s.ink, fontFamily: fontStack(font) }}
        >
          {title}
        </span>
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(160deg, ${s.ink}dd, ${s.primary}bb), linear-gradient(0deg, ${s.soft}, ${s.soft})`,
      }}
    >
      <span
        className="px-3 text-center text-[10px] font-bold leading-tight text-white"
        style={{ fontFamily: fontStack(font) }}
      >
        {title}
      </span>
      <span
        className="h-3 w-12 rounded-full bg-white/90 text-center text-[7px] font-semibold leading-3"
        style={{ color: s.ink }}
      >
        CTA
      </span>
...
export function MiniPageSketch({
  blocks,
  theme,
}: {
  blocks: string[];
    ...
    <div className="flex w-full flex-col gap-1">
      {blocks.map((type, i) => {
        const h =
          type === "hero"
            ? 24
            : type === "courseGrid" || type === "storeProducts"
              ? 16
              : 10;
        const bg =
          type === "hero"
            ...
            {(type === "courseGrid" ||
              ...
              <div className="flex h-full items-center justify-center gap-1 px-2">
                {[0, 1, 2].map((k) => (
                  <span
                    key={k}
                    className="h-3/5 flex-1 rounded-sm bg-white/80"
                  />
                ))}
              </div>
            )}
