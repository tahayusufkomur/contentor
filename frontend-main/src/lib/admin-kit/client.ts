...
import type { ActionResult, ChoiceOption, ListPage, ListParams, ModelMeta, Row, SiteMeta } from './types'
...
export class AdminKitError extends Error {
  ...
  constructor(status: number, data: unknown) {
    ...
    if (!detail && Object.keys(fieldErrors).length === 0) detail = `Request failed (${status})`
...
export function createAdminClient(apiBase: string) {
  ...
  return {
    ...
    retrieve: (model: string, pk: number | string) => kitFetch<Row>(`${base}/${model}/${pk}/`),

    create: (model: string, data: Record<string, unknown>) =>
      kitFetch<Row>(`${base}/${model}/`, { method: 'POST', body: JSON.stringify(data) }),

    update: (model: string, pk: number | string, data: Record<string, unknown>) =>
      kitFetch<Row>(`${base}/${model}/${pk}/`, { method: 'PATCH', body: JSON.stringify(data) }),
