    ...
    fetch('/api/auth/impersonate/verify', {
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(data.detail || 'This impersonation link is invalid or expired.')
        }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-6">
      <div className="text-center">
        {error ? (
          <>
            <h1 className="text-lg font-semibold text-foreground">Couldn’t start the session</h1>
            <p className="mt-2 text-sm text-muted-foreground">{error}</p>
          </>
        ) : (
          <>
            <div className="mx-auto h-6 w-6 animate-spin rounded-full border-2 border-muted border-t-primary" />
            <p className="mt-4 text-sm text-muted-foreground">Opening session…</p>
          </>
        )}
      </div>
    </div>
