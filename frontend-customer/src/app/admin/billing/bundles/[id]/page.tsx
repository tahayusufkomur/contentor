...
function normalizeBundleItemType(contentTypeName?: string, contentTypeId?: number): SelectedItem['content_type'] {
  if (contentTypeName) {
    const mapped = CONTENT_TYPE_NAME_MAP[contentTypeName]
    if (mapped) return mapped
    if (contentTypeName === 'course' || contentTypeName === 'download' || contentTypeName === 'live_class' || contentTypeName === 'live_stream') {
...
export default function EditBundlePage() {
  ...
  const [initialOriginalPrice, setInitialOriginalPrice] = useState<number | null>(null)

  const originalPrice = useMemo(() => {
    const total = selectedItems.reduce((sum, item) => sum + parseFloat(item.price || '0'), 0)
    const hasRealItemPrices = selectedItems.some((item) => parseFloat(item.price || '0') > 0)
    if (!hasRealItemPrices && initialOriginalPrice !== null) return initialOriginalPrice.toFixed(2)
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([bundle, products]) => {
        const productMap = new Map(products.map((p) => [`${p.type}:${p.id}`, p] as const))
        setForm({
          name: bundle.name,
          description: bundle.description ?? '',
          price: bundle.price,
        })
        const parsedOriginalPrice = Number.parseFloat(bundle.original_price ?? '')
        setInitialOriginalPrice(Number.isFinite(parsedOriginalPrice) ? parsedOriginalPrice : null)
        if (bundle.items) {
          setSelectedItems(
            bundle.items.map((item) => {
              const normalizedType = normalizeBundleItemType(item.content_type_name, item.content_type)
              const product = productMap.get(`${normalizedType}:${item.object_id}`)
              return {
