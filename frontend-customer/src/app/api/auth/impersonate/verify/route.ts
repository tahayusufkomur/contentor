...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(new URL('/api/v1/auth/impersonate/verify/', DJANGO_API_URL).toString(), {
      method: 'POST',
      headers: {
    ...
    const data = await res.json().catch(() => ({ detail: 'Verification service unavailable' }))
    ...
    return NextResponse.json({ detail: 'Verification service unavailable' }, { status: 502 })
  }
}
