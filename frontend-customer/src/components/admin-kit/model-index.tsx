...
export function AdminModelIndex({ apiBase, basePath }: { apiBase: string; basePath: string }) {
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof AdminKitError ? err.detail : 'Failed to load.')
      })
    return () => {
      cancelled = true
    }
  ...
  if (error) {
    return (
      <div className="p-4 md:p-6">
        <KitBanner kind="error" message={error} onDismiss={() => setError('')} />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Browse, edit and act on {site.title.toLowerCase()} records.</p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold text-foreground">{model.label_plural}</h2>
              </div>
              {model.description && (
                <p className="mt-3 line-clamp-2 text-sm text-muted-foreground">{model.description}</p>
              )}
            </Link>
          )
        })}
