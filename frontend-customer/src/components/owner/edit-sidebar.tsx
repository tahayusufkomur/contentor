...
import { PAGE_KEYS, PAGE_LABELS, PAGE_ROUTES, pageKeyForPath } from "@/lib/blocks/pages";
import type { TenantConfig } from "@/types/tenant";

type Mode = "site" | "pages";
type SiteSection = "brand" | "navbar";

const SITE_SECTIONS: { id: SiteSection; label: string; icon: React.ReactNode }[] = [
...
export function EditSidebar({ initialConfig, children }: EditSidebarProps) {
  ...
  const [mode, setMode] = useState<Mode>(initialConfig.onboarding_completed ? "pages" : "site");
  const [expanded, setExpanded] = useState<Set<SiteSection>>(() => new Set<SiteSection>(["brand"]));
  ...
  const handleChange = useCallback(
    (patch: Partial<TenantConfig>) => {
      ...
      if (!nextConfig.onboarding_completed) nextConfig.onboarding_completed = true;
      setConfig(nextConfig);
      pendingConfigRef.current = nextConfig;

      if (debounceRef.current) clearTimeout(debounceRef.current);
      debounceRef.current = setTimeout(() => persistConfig(nextConfig), DEBOUNCE_MS);
    },
  ...
  return (
    <div className="relative flex min-h-screen">
      ...
      <aside
        ...
        style={{ width: open ? SIDEBAR_WIDTH : 0, overflow: open ? undefined : "hidden" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b px-5 py-4" style={{ minWidth: SIDEBAR_WIDTH }}>
        ...
        <div className="flex gap-1 border-b p-2" style={{ minWidth: SIDEBAR_WIDTH }}>
          <button
            onClick={() => setMode("site")}
            className={cn(
              "flex flex-1 items-center justify-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium transition-colors",
              mode === "site" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent",
            )}
          >
          ...
          <button
            ...
            className={cn(
              "flex flex-1 items-center justify-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium transition-colors",
              mode === "pages" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent",
            )}
          >
        ...
        <div className="flex-1 overflow-y-auto" style={{ minWidth: SIDEBAR_WIDTH }}>
          {mode === "site" ? (
            <>
                ...
                <div key={section.id} className="border-b last:border-b-0">
                  ...
                  <div
                    className={cn(
                      ...
                      expanded.has(section.id) ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0",
                    )}
                  >
                    <div className="overflow-hidden">
                      <div className="px-5 pb-5 pt-1">
                        {section.id === "brand" ? (
                          <BrandTab config={config} onChange={handleChange} />
                        ) : (
                          <NavbarTab config={config} onChange={handleChange} />
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-5 py-4">
              {/* Page switcher */}
              <p className="mb-2 text-xs font-medium text-muted-foreground">Editing page</p>
              ...
              {activePageKey ? (
                <BlocksTab key={activePageKey} config={config} pageKey={activePageKey} onChange={handleChange} />
              ) : (
                <p className="text-xs text-muted-foreground">
                  This page isn&apos;t editable from the builder. Pick a page above to edit its content.
                </p>
              )}
            </div>
          )}
      ...
      >
        ...
        <style
          dangerouslySetInnerHTML={{
            __html: generateThemeCSS(config.theme, config.font_family, config.custom_css || ""),
          }}
        />
