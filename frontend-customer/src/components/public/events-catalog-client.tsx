...
export function EventsCatalogClient({ events, layout, filterGroupIds }: EventsCatalogClientProps) {
  const [facetSel, setFacetSel] = useState<FacetSelection>({})
  const facets = useMemo(() => buildFacets(events, filterGroupIds), [events, filterGroupIds])
      ...
      <>
        ...
        <div className="space-y-3">
          {filtered.map((event) => {
            ...
            return (
              <Link key={`${event.type}-${event.id}`} href={`/calendar/${event.type}/${event.id}`} className="block">
                <Card className="transition-all hover:shadow-md">
                  <CardContent className="flex flex-wrap items-center gap-x-4 gap-y-2 p-4">
                    <Badge variant="secondary" className="text-xs">
                      {TYPE_LABELS[event.type] ?? event.type}
                    </Badge>
                    <span className="flex-1 font-semibold leading-snug">{event.title}</span>
