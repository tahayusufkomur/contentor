...
| Logo Studio v2 (brief → wall of 24 → canvas editor → brand-kit zip w/ vector SVG; favicon/PWA icons) | partial | the wall-based v2 deployed 2026-07-08 is what's LIVE in prod today; local main (pushed `1aaa0fc`) has since replaced the Ideas step with a curated-first flow (wall removed, curated gallery ranked by brief, server-side PNG→vector tracing, refine keeps the existing icon by default) + made the AI chat path always finish with a real tagline — none of this is deployed yet; paid-tier AI Brand Pack (`5995299`) still inert until a provider key is set |
...
| Shared AI provider (`apps/core/ai.py`, AI_PROVIDER switch, `make ai-check`) | built-not-deployed | MERGED to main and pushed to origin (was an unmerged branch as of the 07-10 stamp — corrected); blog/help-bot/logo routed through it; dev runs `AI_PROVIDER=cli` (subscription auth, works); `.env.prod` has no `AI_PROVIDER` set yet — prod key still not set, so help bot/blog-AI/Brand Pack stay dormant in prod regardless of deploy |
...
- Curated Logo Library Phase 3 (requested 2026-07-12, never designed): (1) coach-admin's "quick logo suggestions" swapped to pull from the curated list, (2) superadmin's `/admin/m/curated-logos` page gets a gallery/thumbnail grid view instead of today's plain text table, (3) drag-and-drop upload + JSON-paste bulk-add replacing the current click-through create form. Internal tooling only — no coach-facing surface, doesn't touch the money path. Do before it if: you're about to hand-curate a large batch of logos and the current table is genuinely slowing you down; otherwise it can wait past launch.
...
Last check: **2026-07-12 (/po next)** — verified: local main == origin/main at `1aaa0fc` (0 ahead/0 behind; today's session pushed 10 Logo Studio commits — curated-first Ideas rework (wall removed, curated gallery ranked by brief, PNG→vector tracing, refine keeps the icon by default) + AI-chat tagline requirement fix — plus merged a concurrent agent's docs-archive commit). Prod `/api/health/` ok (db+redis ok); `/blog` 200; help bot status still `{"enabled":false,"reason":"disabled"}` (unchanged). Corrections: (1) Shared AI provider row was stale — it's MERGED to main (not an unmerged branch as the 07-10 stamp said), `apps/core/ai.py` confirmed present; prod `.env.prod` still has no `AI_PROVIDER` set, so this is now `built-not-deployed` pending the same prod-key step as before. (2) Logo Studio row split: the wall-based v2 deployed 07-08 is what's actually live; today's curated-first rework is pushed but undeployed — row demoted `live-in-prod` → `partial`. Ranking: money-path #1 / secrets #2 / walkthrough #3 unchanged (today's work doesn't touch the launch gate); shared-ai-provider item (#4) simplifies — the merge is done, only the prod key + deploy remain.

Earlier — Last check: **2026-07-10 (/po next)** — verified: main == origin/main at `9520a5e` (0 unpushed); working tree is on branch `feat/shared-ai-provider` (8 commits unmerged + uncommitted ai_check/test polish — shared-tree caution: main is NOT checked out). Prod `/api/health/` ok; **prod is NEWER than the 07-08 stamp**: /blog renders ("Blog — Contentor") and platform blog API 200 → the 22 post-`b95a0b4` commits (blog ×16, help bot ×2, AI brand pack, docs) are pushed AND deployed. Help bot present but OFF in prod (`/api/v1/help/status/` → enabled:false, reason "disabled"). Billing portal endpoint re-greped: still missing. Corrections: checklist boxes 1-2 evidence refreshed; inventory +3 rows (blog, help bot, shared AI provider); logo-v2 row updated for the paid Brand Pack (decision-log addendum recorded from commit `5995299`); custom-domain row marked deployed-inert. Ranking: money path #1 / secrets #2 / walkthrough #3 unchanged (all owner-gated); NEW #4 = land shared-ai-provider then set one prod AI key (activates 3 dormant AI surfaces); failed-payment MVP → #5.
