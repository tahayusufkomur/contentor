...
- 🟡 `UNVERIFIED` — **`django-prometheus` declared, Prometheus scrapes `django:8000`, but the app exposes no `/metrics`** — `backend/requirements/prod.txt:3`. Scrape job permanently empty. *(Resolved 2026-08-03 by removal: dep, dev monitoring profile, and prometheus/loki/grafana configs deleted — the logbook pipeline supersedes them. See P2-C.)*
...
- `[x]` **P2-C · Observability** — `S` — _§E._ Done: `Dockerfile` takes a `PIP_REQUIREMENTS` arg; prod compose installs `prod.txt` for django + celery (verified: prod image has sentry-sdk + django-prometheus, no pytest); `sentry_sdk.init()` wired in prod.py (no-op until `SENTRY_DSN` set), template updated. _2026-08-03: django-prometheus wiring is moot — dep + dev monitoring profile + prometheus/loki/grafana configs removed; the logbook pipeline (`RequestActivityMiddleware` → Vector → `RequestEvent` with status + duration_ms) supersedes a metrics stack at this scale. Error-spike/uptime alerting still to do (backlog'd in PRODUCT.md)._
...
- `[~]` **P4-A · Kill dead code & repo weight** — `S` — _§G/§J/§N._ Done: deleted the 3 dead billing serializers + 8 dead frontend components (455 LOC); archived 32 of 52 `docs/superpowers/specs/` (shipped-and-deployed features) into `docs/superpowers/specs/archive/`, 21 kept active (in-progress/unmerged/undeployed + living tooling docs). **Still open:** archive `docs/superpowers/plans/` (67 files) the same way; un-track the 3.8 MB screenshot-map; delete the 2 zero-commit `claude/*` branches + stale worktrees; flag/gate `apps/domains`; drop `iyzico`/stripe stubs (needs a migration); **rescue the stranded cross-tenant fix in `feat/ai-assistants-v2` before merging/closing it.**
- `[~]` **P4-B · Shared frontend package + de-boilerplate** — `L` — _§G._ Done: `serverFetch` empty-body guard; single `is_coach_or_owner()` predicate (killed the ~14 inline role checks + a private duplicate). **Still open (needs frontend build/browser verification or a migration):** the npm-workspace `packages/shared` for admin-kit + ui primitives + api-client + constants (reconcile drift first, delete `sync-admin-kit.sh`); collapse the 4 AI usage models (migration); extract demo seeding + logo engine out of the god-apps; typed settings schema.
  - _Gotcha found: dev and prod compose both name the image `contentor-django`, so `docker compose -f docker-compose.prod.yml build` clobbers the running dev image (dev then loses pytest/ruff until `docker compose build django` rebuilds it). Give the prod image a distinct name or use a separate project name for prod builds._
