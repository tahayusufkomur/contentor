...
- `[ ]` **P0-C · Live-app access checks** — `M` — _§B._ Gate Zoom link/meeting-id, on-site address, and Stream chat/video token to the specific class + enrolment; restrict Stream `host` role to the instructor. Verify e2e: enrolled vs anonymous vs cross-tenant.
- `[ ]` **P0-D · Webhook idempotency ordering** — `M` — _§D crit._ Commit the dedup record only after successful processing (or `processing`→`done`/`failed`); move Stripe network calls + `.delay()` out of the atomic transaction (`on_commit`). Verify with `stripe trigger` fail-then-succeed.
