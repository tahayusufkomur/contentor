...
- `[~]` **P4-A · Kill dead code & repo weight** — `S` — _§G/§J/§N._ Done: deleted the 3 dead billing serializers + 8 dead frontend components (455 LOC). **Still open:** archive `docs/superpowers` (119 files); un-track the 3.8 MB screenshot-map; delete the 2 zero-commit `claude/*` branches + stale worktrees; flag/gate `apps/domains`; drop `iyzico`/stripe stubs (needs a migration); **rescue the stranded cross-tenant fix in `feat/ai-assistants-v2` before merging/closing it.**
