...
- `[ ]` **P0-A · Database backup + recovery** — `S` — _Critical E._ Cron `pg_dumpall` → gzip → offsite (R2/S3), daily + retention, with a one-command restore runbook + a media-bucket backup. Verify by restoring into a throwaway container and diffing row counts. **Do first.** (Cron install + offsite creds are a manual prod step.)
- `[ ]` **P0-B · Stored-XSS sanitization** — `M` — _§C._ Sanitize on write **and** render for `custom_css` (`tenant-theme-style.tsx:12`), blog `body_html` (`blog/serializers.py:30`), lesson `content_html` (`courses/serializers.py:332`). Verify with `<script>`/`onerror` payloads through the coach forms.
