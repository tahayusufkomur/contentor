...
// `sharp` is an optional peer dep and supplies the only image codec the CLI can use.
// `pnpm dlx <spec>` does not install optional peers, so the bare spec fails at runtime with
// "Image codec is unavailable" — co-install it and invoke the `gwr` bin explicitly.
function buildDlxArgs(args) {
  return ['dlx', `--package=${DEFAULT_CLI_PACKAGE_SPEC}`, '--package=sharp', 'gwr', ...args];
}
...
function getPathFallbackCandidates(args) {
  if (process.platform === 'win32') {
    ...
    if (hasPathCommand('pnpm')) {
      candidates.push(buildWindowsShellCandidate('pnpm', buildDlxArgs(args)));
    }
    return candidates;
  }
  ...
  if (hasPathCommand('pnpm')) {
    candidates.push({ command: 'pnpm', commandArgs: buildDlxArgs(args) });
  }
  return candidates;
}
