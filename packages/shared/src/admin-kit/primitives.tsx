...
export const KitInput = forwardRef<
  HTMLInputElement,
...
export function KitSelect({
  ...
  return (
    <select
      className={`${CONTROL} appearance-none pr-8 ${className}`}
      {...props}
    />
  );
}
...
export function KitBanner({
  ...
  return (
    <div
      className={`flex items-start justify-between gap-3 rounded-md border px-4 py-2.5 text-sm ${tone}`}
    >
      <p>{message}</p>
      <button
        type="button"
